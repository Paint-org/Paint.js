...
module Paint {
    export class Paper {
        ...
        private _layers: Paint.PaperLayer[] = [];
        private _paper: HTMLElement;
        private _savedCanvas: HTMLCanvasElement[] = [];

        constructor(paint: Paint.Global, element: HTMLElement) {

            this._paint = paint;
            this._paper = element;

            this.addLayer(Paint.Color.White);
            this.setHandlers();
        }
        ...
        get baseLayer(): Paint.PaperLayer {
            return this._layers[0];
        }

        get layers(): Paint.PaperLayer[] {
            return this._layers;
        }
         ...
         */
        addLayer(background: Paint.Color): Paint.PaperLayer {
            var $ = this._paint.$;
        ...
        newCanvas.css('position', 'absolute');
            ...
            var layer = new Paint.PaperLayer(this._paint, <HTMLCanvasElement>newCanvas[0], background);
         ...
         * Removes a layer.
         */
        removeLayer(layer: Paint.PaperLayer) {
        ...
        pageXYtoPaperXY(x: number, y: number): Paint.Point {
            var offset = this._paint.$(this._paper).parent().offset();
            return new Paint.Point(
